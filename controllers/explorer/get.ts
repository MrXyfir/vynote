...
export = (socket: SocketIO.Socket, fn: Function) => {
    ...
    db(cn => {
        ...
        let sql: string = `
            SELECT parent_id, folder_id, name FROM folders WHERE user_id = ?
        `;
        ...
        cn.query(sql, [socket.session.uid], (err, rows) => {
            ...
            sql = `
                ...
                created, syntax, 0 as contributor FROM documents WHERE user_id = ?
            `;
            ...
            cn.query(sql, [socket.session.uid], (err, rows) => {
                ...
                sql = `
                    ...
                    documents.created, documents.syntax, document_contributors.folder_id, 1 as contributor 
