...
export = (socket: SocketIO.Socket, folder: number, fn: Function) => {
    ...
    db(cn => {
        ...
        let sql: string = "SELECT parent_id, folder_id, name, color "
        ...
        cn.query(sql, [socket.session.uid, folder], (err, rows) => {
            ...
            sql = "SELECT doc_type, doc_id, folder_id, name, encrypted, "
                + "created, thumbail, meta FROM documents "
                + "WHERE user_id = ? AND parent_id = ?";
            cn.query(sql, [socket.session.uid, folder], (err, rows) => {
                response.documents = rows;

                // Grab documents that user is a contributor on
                sql = "SELECT doc_type, doc_id, name, encrypted, "
                    + "created, thumbail, meta FROM documents WHERE doc_id IN ("
                    + "SELECT doc_id FROM document_contributors WHERE user_id = ? AND parent_id = ?)";
                cn.query(sql, [socket.session.uid, folder], (err, rows) => {
                    cn.release();
